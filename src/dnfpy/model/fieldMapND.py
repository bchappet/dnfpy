...
class FieldMapND(MapND):
    def __init__(self,name,size,dim=1,dt=0.1,model='cnft',tau=0.64,h=0.0,th=0.75,delta=1.,
        ...
        super(FieldMapND,self).__init__(name,size,dim=dim,dt=dt,model=model,
                                      ...
                                      afferentInhibition=afferentInhibition,wAffInh=wAffInh,**kwargs)

        self.noise = NoiseMap(name+"_noise",size=size,dim=dim,dt=dt,intensity=noiseI)
        self.addChildren(noise=self.noise)


    def _compute(self,model,lat,aff,dt,tau,h,th,delta,resetLat,gainAff,noise,afferentInhibition,wAffInh):
        if model == 'cnft':
            self._data = self._data + dt/tau*(-self._data + h + aff*gainAff + delta*lat - afferentInhibition*wAffInh) + noise
        elif model == 'spike':
