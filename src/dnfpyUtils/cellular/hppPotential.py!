...
class ActMap(Map2D):
    ...
    """
    def __init__(self,name,size,dt=0.1,th=3.5,**kwargs):
        super(ActMap,self).__init__(name,size,dt=dt,th=th,dtype=np.bool_,**kwargs)

    def _compute(self,pot,th):
...
class HppPotential(CellularMap):
    ...
    """
    def __init__(self,name,size,dt=0.1,alpha=0.1,th=3.5,**kwargs):
        super(PotMap,self).__init__(name,size,dt=dt,dtype=np.float,alpha=alpha,th=th,**kwargs)
        self.activation = ActMap("act",size,dt=dt,th=th)
        self.activation.addChildren(pot=self)

        self.hppExc.addChildren(activation=self.activation,potential=self)
        self.hppInh.addChildren(activation=self.activation,potential=self)
        self.addChildren(hpp=self.hppExc,activation=self.activation)

    def _compute(self,activation,hpp,alpha,th):
        
        #reset pot on activation
        self._data = np.where(activation,-1,self._data)
        #self.obs = np.roll(self.obs,1,axis=1)
        somme = np.sum(hpp,axis=2).astype(np.uint8)
        #mean = np.mean(somme)
        #kernel = np.ones((3,3),dtype=np.uint8)
        #kSum = np.sum(kernel)
        ...
        toAdd = np.where(somme > 2 ,alpha,-alpha)
        self._data += toAdd
        self._data = np.clip(self._data,-1,th+1)


    def getArrays(self):
        return [self.activation,self.hppExc]
...
class Hpp2(CellularMap):
    ...
    def _compute(self,activation,th):
            self.compute(activation,th)

    @profile
    def compute(self,activation,th):
        self.lock.acquire()
